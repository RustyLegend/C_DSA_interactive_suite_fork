...
void help_hashing_menu(void)
{
    ...
    printf("--- INTRODUCTION TO HASHING & COLLISIONS ---\n\n");
    printf("CONCEPT:\n");
    ...
    printf("    A function that converts a given key into a table index. A good hash\n");
    printf("    function distributes keys uniformly across the table to minimize collisions.\n\n");
    printf("COLLISIONS:\n");
    printf("    A collision occurs when two different keys map to the same table index.\n");
    printf("    Since multiple keys cannot occupy the same slot, collision resolution\n");
    printf("    methods are required to handle this.\n\n");

    printf("--- LINEAR PROBING (OPEN ADDRESSING) ---\n\n");
    printf("LINEAR PROBING:\n");
    printf("    A method of collision resolution in Open Addressing. When a collision\n");
    printf("    occurs at index i, the table is searched sequentially (one slot at a time)\n");
    ...
    printf("    • Suffers from Primary Clustering (long contiguous runs of occupied slots\n");
    printf("      forming, which increases search times).\n\n");

    printf("--- SEPARATE CHAINING (CLOSED ADDRESSING) ---\n\n");
    printf("SEPARATE CHAINING:\n");
    printf("    A method of collision resolution using Closed Addressing. Instead of\n");
    printf("    finding another slot in the table, each slot contains a pointer to a\n");
    printf("    linked list of keys that map to the same index.\n\n");
    printf("CHARACTERISTICS:\n");
    printf("    • Table size never restricts the number of elements (grows dynamically).\n");
    printf("    • Simple insertion (insert at head/tail of the linked list).\n");
    printf("    • Requires extra pointer memory for lists.\n");
    printf("    • Worst-case search time degrades to O(N) if all elements hash to the\n");
    printf("      same slot (forming a single long linked list).\n\n");

    printf("--- QUADRATIC PROBING (OPEN ADDRESSING) ---\n\n");
    printf("QUADRATIC PROBING:\n");
    printf("    An Open Addressing method designed to eliminate primary clustering.\n");
    printf("    Instead of searching sequentially, it searches slots using quadratic offsets.\n\n");
    printf("PROBING FORMULA:\n");
    printf("    hash(key, i) = (hash(key) + c1 * i + c2 * i^2) %% table_size\n");
    printf("    where c1, c2 are constants and i = 0, 1, 2, ... is the probe number.\n\n");
    printf("CHARACTERISTICS:\n");
    printf("    • Eliminates Primary Clustering.\n");
    printf("    • Suffers from Secondary Clustering (keys hashing to the same initial\n");
    printf("      index will follow the exact same probe sequence).\n");
    printf("    • Table size must be a prime number to guarantee finding a free slot.\n\n");

    printf("--- DOUBLE HASHING (OPEN ADDRESSING) ---\n\n");
    printf("DOUBLE HASHING:\n");
    printf("    One of the best Open Addressing methods. It uses a second hash function\n");
    printf("    to compute the step size for probing when a collision occurs, ensuring the\n");
    ...
    printf("      - Pros: Better Cache Locality (elements are contiguous in memory),\n");
    printf("              no pointer overhead.\n");
    printf("      - Cons: Table can get full; performance degrades rapidly as the load\n");
    printf("              factor (alpha = N/M) approaches 1.0. Suffers from Clustering.\n");
    printf("    • Closed Addressing (Separate Chaining):\n");
    printf("      Elements mapping to the same bucket are linked outside the table.\n");
    printf("      - Pros: Table never fills up; degrades gracefully as elements increase.\n");
    printf("      - Cons: Poorer Cache Locality (requires pointer chasing through memory),\n");
    ...
    printf("        whereas open addressing requires dummy placeholders/tombstones).\n");
    ...
    printf("      - High cache locality performance is critical (reads/writes are fast).\n");
    printf("      - Load factor is guaranteed to stay low (typically < 0.7).\n");
    printf("    • Use Double Hashing when:\n");
    printf("      - You want open addressing but want to avoid clustering completely.\n");
    printf("      - Load factor is high, and a second hash computation overhead is minor.\n\n");
    printf("REAL-WORLD APPLICATIONS:\n");
    printf("    • Java's java.util.HashMap uses Separate Chaining (which converts to a\n");
    ...
    printf("      sequence (similar to quadratic probing/double hashing) to maintain\n");
    printf("      extremely fast cache-friendly lookups.\n");
    printf("    • C++ std::unordered_map is typically implemented using Separate Chaining.\n");
    printf("    • Database Indexes and Routers often use Double Hashing or Linear Probing\n");
